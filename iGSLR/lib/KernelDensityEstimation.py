...
class KernelDensityEstimation(object):
    ...
    def f(self, dij, u):
        return 1.0 * np.sum([self.K((dij - d) / self.h[u])
                             for d in self.d[u]]) / len(self.d[u]) / self.h[u]

    def predict(self, u, lj):
        if u in self.h and not self.h[u] == 0:
            lat_j, lng_j = self.poi_coos[lj]
            return np.sum([cKDE.prob(lat_i, lng_i, lat_j, lng_j, self.d_cpp[u], len(self.d[u]), self.h[u])
                           for lat_i, lng_i in self.L[u]]) / len(self.L[u])
        return 1.0
